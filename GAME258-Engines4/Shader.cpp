...
void Shader::readCompileAttach(const char *vsFilename, const char *fsFilename){
	...
	try { 		
		...
		if(vsText) delete[] vsText, vsText = nullptr;
		if(fsText) delete[] fsText, fsText = nullptr;
...
GLuint Shader::getUniformID(std::string name) { 
	...
	if (id == uniformMap.end()) {
		...
		///Using a -1 for the location in any glUniform... will ignore the data sent to it 
		return -1;
	}
...
void Shader::setUniformLocations(){
	...
	printf("There are %o active Uniforms\n",count);
